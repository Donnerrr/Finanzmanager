...
async function loadPersonsFromDB() {
    ...
    try {
        const response = await fetch('https://172.19.192.1:7033/api/Schuldenbuch/Person');
...
async function loadPersonDetails(personId) {
    ...
    try {
        const response = await fetch(`https://172.19.192.1:7033/api/Schuldenbuch/Person/${personId}`);
...
async function saveEntry(event) {
    ...
    try {
        const response = await fetch('https://172.19.192.1:7033/api/Schuldenbuch/Debt', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function deleteDebt(debtId) {
    if (confirm("Sind Sie sicher, dass Sie diese Schuld löschen möchten?")) {
        ...
        const response = await fetch(`https://172.19.192.1:7033/api/Schuldenbuch/Debt/${debtId}`, {
            method: 'DELETE'
        });
